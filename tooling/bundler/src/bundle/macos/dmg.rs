...
use std::{env, fs::{write}, process::{Command, Stdio}};

#[cfg(not(target_os = "linux"))]
use crate::common::CommandExt;

#[cfg(target_os = "linux")]
use super::dmg_linux::make_dmg_in_linux;
...
pub fn bundle_project(settings: &Settings, bundles: &[Bundle]) -> crate::Result<Bundled> {
  ...
  // TODO: if hdiutil is not available, assume linux and run code in dmg_linux
  #[cfg(target_os = "linux")]
  {
    let icns_icon_path =
